...
function setup_storage_host() {
    ...
    while true; do
        read -p "Введите имя хоста хранилища (например backup-server): " storage_host
        
        if [ -z "$storage_host" ]; then
            log "Имя хоста не может быть пустым!" "ERROR"
            continue
        fi
...
# Функция для настройки глобальных параметров с проверками (ИСПРАВЛЕННАЯ)
...
function create_host_configs() {
    ...
    for hostname in "${!host_segments[@]}"; do
        # Пропускаем создание отдельного конфига для хранилища
        if [ "$hostname" == "$storage_host" ]; then
            continue
        fi
        
        local host_config="$host_global_section\n\n${host_segments[$hostname]}"
...
function deploy_hosts_list() {
    if [ -z "$storage_host" ]; then
        log "Хост хранилища не установлен!" "ERROR"
        return 1
    fi
...
# Функция для доставки конфигов на хосты (ИСПРАВЛЕННАЯ)
function deploy_configs() {
    ...
    # Добавляем хост хранилища в список
    ...
    for host in $db_hosts; do
        ...
        if [ "$host" == "$storage_host" ]; then
            # Для хранилища - основной конфиг
            config_file="$config_dir/pgbackrest.conf"
        elif [ "$host" == "$current_host" ]; then
            # Для мастера - его специфичный конфиг
            config_file="$config_dir/pgbackrest-$host.conf"
        else
            # Для остальных сегментов - их специфичные конфиги
            config_file="$config_dir/pgbackrest-$host.conf"
        fi
    ...
    # Отправляем файл со списком хостов на хранилище
    deploy_hosts_list
